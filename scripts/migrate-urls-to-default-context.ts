...
async function migrateUrlsToDefaultContext() {
  console.log('🔄 Migrating URLs to Default Context for Pro Mode Users');
  ...
  try {
    ...
    for (const user of allUsers) {
      ...
      if (user.proMode) {
        ...
        if (activeProfiles.length > 0) {
          ...
          // Remove URLs from main table since they're now in context
          await db.delete(urls).where(eq(urls.userId, user.id));
          console.log(`  - ✅ Removed ${migratedUrls.length} URLs from main table`);
    ...
    for (const user of allUsers) {
      ...
      if (user.proMode && userMainUrls.length > 0) {
        console.log(`  - ⚠️  Pro mode user has URLs in main table (these are in default context)`);
      }
    }
    
    console.log(`\n✅ Migration Complete!`);
    console.log(`\n📝 How this works:`);
    console.log(`• Pro mode users with active profiles: URLs moved to profile context`);
...
// Run the migration
migrateUrlsToDefaultContext().catch(console.error); 